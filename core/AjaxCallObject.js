...
__AjaxControlFocus = new String();
function AjaxCallObject()
{
  this.Init();
}
AjaxCallObject.prototype.SetFocusOfControl = function(clientID)

{
__AjaxControlFocus = clientID;
...
AjaxCallObject.prototype.OnComplete = function(responseText, responseXml)
{
    ...
    if (__AjaxControlFocus != '')

    {
    var place = document.getElementById(__AjaxControlFocus); 
    if (place == null)
    {
    place = AJAXCbo.GetTargetElement(__AjaxControlFocus);
    }
    if (place)
    {
    try
...
function CreateWaitElement() {
    ...
    if (!elem) {
        ...
        elem.innerHTML ='<img src="../imagens/loading.gif" /> ';
